...
extension SettingsStore {
    ...
    /// Settings that should trigger provider/status refresh work. Presentation-only
    /// fields stay on `menuObservationToken` so UI changes do not eagerly kick off
    /// network/process work.
    var dataRefreshObservationToken: Int {
        _ = self.providerEnablement
        _ = self.refreshFrequency
        _ = self.debugDisableKeychainAccess
        _ = self.debugKeepCLISessionsAlive
